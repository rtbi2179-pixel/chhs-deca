...
function AuthenticatedOverview({ user }: { user: NonNullable<ReturnType<typeof useAuth>['user']> }) {
  ...
  const practiceMetrics = [
    { label: 'Questions answered', value: metrics ? metrics.questionsAnswered.toLocaleString() : '—', icon: BookOpenCheck, detail: metrics ? 'Across practice and review' : 'Syncing your practice record', tone: 'blue', cluster: 'business-management' },
    { label: 'Overall accuracy', value: metrics ? `${metrics.accuracyPercent}%` : '—', icon: Target, detail: metrics ? `${metrics.correctAnswers.toLocaleString()} correct answers` : 'Syncing your accuracy', tone: 'green', cluster: 'finance' },
    { label: 'Study streak', value: metrics ? `${metrics.studyStreak} day${metrics.studyStreak === 1 ? '' : 's'}` : '—', icon: Activity, detail: 'Practice on consecutive days', tone: 'yellow', cluster: 'hospitality-and-tourism' },
    { label: 'Saved questions', value: metrics ? metrics.savedQuestions.toLocaleString() : '—', icon: Star, detail: 'Ready for focused review', tone: 'slate', cluster: 'marketing' },
  ]
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[oklch(0.07_0.01_265)] text-white">
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        ...
        <section aria-labelledby="study-progress-heading" data-overview-cluster="business-management" className="mt-8">
        ...
        {primaryEvent && <section aria-labelledby="roadmap-heading" data-overview-cluster="finance" className="overview-support-panel overview-cluster-section mt-5 rounded-2xl border border-blue-300/20 bg-[linear-gradient(135deg,oklch(0.17_0.07_255/0.56),oklch(0.085_0.018_265/0.9))] p-6 sm:p-7"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><div className="flex items-center gap-2 text-[10px] font-mono-data uppercase tracking-[0.16em] text-blue-200/75"><Compass className="h-3.5 w-3.5" />Your DECA roadmap</div><h2 id="roadmap-heading" className="mt-2 font-display text-3xl tracking-tight">{roadmap ? `THIS WEEK · ${roadmap.eventCode}` : 'SET YOUR START DATE'}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/65">{roadmap ? roadmapNextTask ? `Next: ${roadmapNextTask.title}` : 'Your current weekly milestones are complete. Review your upcoming competition phase.' : 'Tell Blue Blazer when you began preparing so it can build a saved, deadline-aware path for your selected event.'}</p></div><Link href="/timeline" className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200">{roadmap ? 'Open weekly roadmap' : 'Build roadmap'} <ArrowRight className="h-4 w-4" /></Link></div>{roadmap ? <div className="mt-6 grid gap-3 border-t border-white/10 pt-5 sm:grid-cols-4"><div><p className="text-[10px] font-mono-data uppercase tracking-[0.14em] text-white/45">Readiness</p><p className="mt-1 text-xl font-semibold text-white">{roadmap.readinessScore}%</p></div><div><p className="text-[10px] font-mono-data uppercase tracking-[0.14em] text-white/45">This week</p><p className="mt-1 text-xl font-semibold text-white">{roadmap.thisWeekItems?.length ?? 0}</p></div><div><p className="text-[10px] font-mono-data uppercase tracking-[0.14em] text-white/45">Weekly progress</p><p className="mt-1 text-xl font-semibold text-white">{roadmap.weeklyProgressPercent ?? 0}%</p></div><div><p className="text-[10px] font-mono-data uppercase tracking-[0.14em] text-white/45">Training style</p><p className="mt-1 text-sm font-semibold capitalize text-white">{roadmap.trainingIntensity?.replace("_", " ") ?? "competitive"}</p></div></div> : <p className="mt-5 text-xs leading-5 text-blue-100/70">Your event selection is saved. The preparation date is only requested once and stays attached to this event’s roadmap.</p>}</section>}
        ...
        <div className="mt-8 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <section aria-labelledby="accounts-heading" data-overview-panel="financial" data-overview-cluster="finance" className="overview-support-panel overview-cluster-section rounded-2xl border border-white/10 bg-slate-950/65 p-6 sm:p-7">
            ...
            <div className="mt-6 divide-y divide-white/10 border-y border-white/10"><div data-overview-cluster="business-management" className="overview-account-row flex items-center justify-between py-4"><span className="flex items-center gap-2 text-sm text-white/65"><WalletCards className="h-4 w-4 text-blue-300" />Checking</span><span className="font-mono-data text-sm text-white">{balanceReady ? formatCurrency(bankAccount?.checkingBalance) : 'Syncing…'}</span></div><div data-overview-cluster="personal-finance" className="overview-account-row flex items-center justify-between py-4"><span className="flex items-center gap-2 text-sm text-white/65"><PiggyBank className="h-4 w-4 text-blue-300" />Savings</span><span className="font-mono-data text-sm text-white">{balanceReady ? formatCurrency(bankAccount?.savingsBalance) : 'Syncing…'}</span></div><div data-overview-cluster="finance" className="overview-account-row flex items-center justify-between py-4"><span className="flex items-center gap-2 text-sm text-white/65"><TrendingUp className="h-4 w-4 text-blue-300" />BBX value</span><span className="font-mono-data text-sm text-white">{portfolioReady ? formatCurrency(portfolio?.totalValue) : 'Syncing…'}</span></div></div>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <section aria-labelledby="blues-news-heading" data-overview-panel="news" data-overview-cluster="marketing" className="overview-support-panel overview-cluster-section rounded-2xl border border-white/10 bg-slate-950/65 p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-mono-data uppercase tracking-[0.16em] text-blue-200/65">Market simulation</p><h2 id="blues-news-heading" className="mt-1 font-display text-3xl tracking-tight">BLUE&apos;S NEWS</h2></div><Newspaper className="h-5 w-5 text-blue-300" /></div><div className="mt-6 rounded-xl border border-white/10 bg-white/[0.035] p-4"><p className="text-[10px] font-mono-data uppercase tracking-[0.14em] text-blue-200/60">{recentNewsQuery.isLoading ? 'Checking for updates' : latestNews?.isRead ? 'Latest read update' : `${unreadNewsQuery.data?.count ?? 0} unread update${unreadNewsQuery.data?.count === 1 ? '' : 's'}`}</p><p className="mt-3 text-sm font-semibold leading-6 text-white">{latestNews?.headline ?? (recentNewsQuery.isLoading ? 'Loading Blue’s News…' : 'No active Blue’s News article yet.')}</p><p className="mt-2 line-clamp-2 text-xs leading-5 text-white/55">{latestNews?.body ?? 'New simulated market stories will appear here as they are published.'}</p></div><Link href="/blues-news" data-overview-cluster="marketing" className="overview-cluster-link mt-4 inline-flex items-center gap-2 text-xs font-semibold text-blue-300 transition-colors hover:text-blue-200">Read Blue&apos;s News <ArrowRight className="h-3.5 w-3.5" /></Link></section>

          <section aria-labelledby="chapter-hub-heading" data-overview-panel="chapter" data-overview-cluster="entrepreneurship" className="overview-support-panel overview-cluster-section rounded-2xl border border-white/10 bg-slate-950/65 p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-mono-data uppercase tracking-[0.16em] text-blue-200/65">Stay connected</p><h2 id="chapter-hub-heading" className="mt-1 font-display text-3xl tracking-tight">CHAPTER HUB</h2></div><Building2 className="h-5 w-5 text-blue-300" /></div><div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Link href="/events" data-overview-cluster="hospitality-and-tourism" className="overview-cluster-action rounded-xl border border-white/10 bg-white/[0.035] p-4 transition-colors hover:bg-white/[0.08] focus:outline-none focus:ring-2 focus:ring-blue-300"><Trophy className="h-4 w-4 text-blue-300" /><p className="mt-3 text-sm font-semibold">Events</p><p className="mt-1 text-xs text-white/45">Explore event pathways</p></Link><Link href="/calendar" data-overview-cluster="business-management" className="overview-cluster-action rounded-xl border border-white/10 bg-white/[0.035] p-4 transition-colors hover:bg-white/[0.08] focus:outline-none focus:ring-2 focus:ring-blue-300"><Calendar className="h-4 w-4 text-blue-300" /><p className="mt-3 text-sm font-semibold">Calendar</p><p className="mt-1 text-xs text-white/45">Plan chapter dates</p></Link><Link href="/announcements" data-overview-cluster="marketing" className="overview-cluster-action rounded-xl border border-white/10 bg-white/[0.035] p-4 transition-colors hover:bg-white/[0.08] focus:outline-none focus:ring-2 focus:ring-blue-300"><CircleDollarSign className="h-4 w-4 text-blue-300" /><p className="mt-3 text-sm font-semibold">Announcements</p><p className="mt-1 text-xs text-white/45">Chapter updates</p></Link><Link href="/discussions" data-overview-cluster="entrepreneurship" className="overview-cluster-action rounded-xl border border-white/10 bg-white/[0.035] p-4 transition-colors hover:bg-white/[0.08] focus:outline-none focus:ring-2 focus:ring-blue-300"><Users className="h-4 w-4 text-blue-300" /><p className="mt-3 text-sm font-semibold">Discussions</p><p className="mt-1 text-xs text-white/45">Connect with members</p></Link></div></section>
        </div>
      </div>
