...
export default function CompetitionTimeline() {
  ...
  if (!timeline.data?.timeline) {
    ...
    return <main className="page-shell"><div className="page-content max-w-4xl"><section className="editorial-panel px-6 py-14 text-center sm:px-10"><Target className="mx-auto h-9 w-9 text-blue-300" /><p className="data-label mt-5">Personal competition preparation</p><h1 className="page-title mt-3">{needsStart ? "When are you starting DECA preparation?" : "Build Your Competition Roadmap"}</h1><p className="mx-auto mt-4 max-w-2xl leading-7 text-foreground/70">{needsStart ? `Blue Blazer will save this starting point for ${timeline.data?.eventCode} and build a deadline-aware roadmap around your chapter calendar.` : "Choose your DECA event and Blue Blazer will create an event-aware preparation timeline using your chapter dates and recorded study progress."}</p>{needsStart ? <div className="mx-auto mt-7 max-w-md space-y-3"><Button className="w-full" disabled={startTimeline.isPending} onClick={() => startTimeline.mutate({ startDate: new Date().toISOString().slice(0, 10) })}>Start today</Button><Button className="w-full" variant="outline" onClick={() => setChooseStartDate((value) => !value)}>Choose another date</Button>{chooseStartDate && <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-left"><label className="text-sm text-foreground/75">Preparation start date<input type="date" max={new Date().toISOString().slice(0, 10)} value={startDate} onChange={(event) => setStartDate(event.target.value)} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2 text-foreground" /></label><Button className="mt-3 w-full" disabled={!startDate || startTimeline.isPending} onClick={() => startTimeline.mutate({ startDate })}>Create my roadmap</Button></div>}</div> : <Link href="/events" className="mt-7 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-500">Choose My Event <ArrowUpRight className="h-4 w-4" /></Link>}</section></div></main>;
  }
