...
function uploadMimeType(mimeType: string): "audio/webm" | "audio/ogg" | "audio/mp4" | "audio/mpeg" | "audio/wav" | "video/webm" | "video/mp4" {
...
export default function RoleplayAI() {
  ...
  const [recordingHasVideo, setRecordingHasVideo] = useState(false);
  ...
  function beginSignalMeter(stream: MediaStream) {
    ...
    const read = () => {
      ...
      const level = samples.reduce((sum, sample) => sum + Math.abs(sample - 128), 0) / (samples.length * 128);
      setMicrophoneLevel(Math.min(1, level * 7));
      meterFrameRef.current = window.requestAnimationFrame(read);
    };
  ...
  async function checkDevices() {
    try {
      if (!navigator.mediaDevices?.getUserMedia) throw new Error("Your browser does not support the required microphone permission check.");
      streamRef.current?.getTracks().forEach((track) => track.stop());
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: cameraEnabled });
      streamRef.current = stream;
      setRecordingHasVideo(stream.getVideoTracks().length > 0);
      if (previewRef.current) previewRef.current.srcObject = stream;
      beginSignalMeter(stream);
      setDeviceChecked(true);
      setError(null);
    } catch (cause: any) {
      const name = cause?.name;
      setDeviceChecked(false);
      setError(name === "NotAllowedError" ? "Blue Blazer needs microphone access to record and evaluate your presentation. Allow it in browser settings, then retry the device check." : name === "NotFoundError" ? "No microphone was found. Connect or enable one, then retry the device check." : cause?.message || "Blue Blazer could not check your recording devices.");
    }
  }
  ...
  async function beginRecording() {
    ...
    try {
      ...
      const stream = streamRef.current ?? await navigator.mediaDevices.getUserMedia({ audio: true, video: cameraEnabled });
      const mimeType = (cameraEnabled
...
function Judge({ detail, onBack, onCheck, onRecord, cameraEnabled, setCameraEnabled, setDeviceChecked, microphoneLevel, deviceChecked, busy }: any) {
  const intro = detail?.judgeTurns?.find((turn: any) => turn.turnType === "introduction")?.question;
  return <section className="space-y-5"><SectionHeading number="04" eyebrow="Competition setup" title="Check your recording setup, then meet the judge." text="Blue Blazer asks for microphone permission only after you choose to test it. Camera remains optional for roleplay and is never used for a visual score unless an analysis service explicitly becomes available." /><div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]"><div className="rounded-2xl border border-white/10 bg-slate-950/55 p-5"><p className="font-mono-data text-[10px] uppercase tracking-[0.16em] text-blue-200/70">Recording mode</p><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => { setCameraEnabled(false); setDeviceChecked(false); }} className={`rounded-xl border p-3 text-left ${!cameraEnabled ? "border-blue-300/45 bg-blue-400/[0.09]" : "border-white/10 bg-white/[0.025]"}`}><Mic className="h-4 w-4 text-blue-200" /><p className="mt-2 text-xs font-semibold text-white">Audio only</p><p className="mt-1 text-[11px] leading-4 text-slate-400">Default roleplay mode</p></button><button type="button" onClick={() => { setCameraEnabled(true); setDeviceChecked(false); }} className={`rounded-xl border p-3 text-left ${cameraEnabled ? "border-blue-300/45 bg-blue-400/[0.09]" : "border-white/10 bg-white/[0.025]"}`}><Camera className="h-4 w-4 text-blue-200" /><p className="mt-2 text-xs font-semibold text-white">Audio + camera</p><p className="mt-1 text-[11px] leading-4 text-slate-400">Stored for playback; visual analysis unavailable</p></button></div><div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-3"><div className="flex items-center justify-between text-xs"><span className="text-slate-300">Microphone signal</span><span className={deviceChecked ? "text-emerald-200" : "text-slate-500"}>{deviceChecked ? microphoneLevel > 0.02 ? "Good signal" : "Speak to test" : "Not checked"}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-emerald-400 transition-[width]" style={{ width: `${Math.round(microphoneLevel * 100)}%` }} /></div></div><Button variant="outline" onClick={onCheck} disabled={busy} className="mt-4 w-full border-blue-300/25 text-blue-100 hover:bg-blue-400/10"><Volume2 className="mr-2 h-4 w-4" />{deviceChecked ? "Retest microphone" : "Test microphone"}</Button></div><div className="rounded-2xl border border-blue-300/20 bg-[radial-gradient(circle_at_80%_0%,rgba(59,130,246,0.18),transparent_38%),rgba(2,8,23,0.82)] p-7 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-blue-300/25 bg-blue-400/10"><Volume2 className="h-6 w-6 text-blue-200" /></div><p className="mt-5 font-mono-data text-[10px] uppercase tracking-[0.2em] text-blue-200/70">{detail?.scenario?.judgeRole}</p><p className="mx-auto mt-4 max-w-2xl text-lg leading-8 text-white sm:text-xl">“{intro || "Thank you for meeting with me. Please walk me through your recommendation."}”</p></div></div><div className="flex justify-between gap-3"><Button variant="outline" onClick={onBack} className="border-white/15 text-slate-300"><ArrowLeft className="mr-2 h-4 w-4" />Return to notes</Button><Button onClick={onRecord} disabled={busy || !deviceChecked} className="bg-rose-500 text-white hover:bg-rose-400"><Mic className="mr-2 h-4 w-4" />Start recording</Button></div></section>;
}

function Record({ detail, remaining, live, seconds, url, prompt, mode, summary, setSummary, onRecord, onStop, microphoneLevel, hasVideo, previewRef, onFollowUp, onReview, busy }: any) {
  return <section className="space-y-5"><SectionHeading number="05" eyebrow="Timed interview" title="Present, respond, and stay specific." text="Your original recording is preserved before a server derives acoustic delivery evidence and a supporting transcript. Competition mode does not show live captions." /><div className="grid gap-5 lg:grid-cols-[0.78fr_1.22fr]"><div className="rounded-2xl border border-rose-300/20 bg-[linear-gradient(145deg,rgba(70,12,30,0.4),rgba(2,8,23,0.92))] p-5"><div className="flex items-center gap-2 font-mono-data text-[10px] uppercase tracking-[0.18em] text-rose-100/80"><span className={`h-2 w-2 rounded-full ${live ? "animate-pulse bg-rose-300" : "bg-slate-600"}`} />{live ? "Recording" : "Interview window"}</div><p className={`mt-3 font-mono-data text-6xl font-semibold ${remaining === 0 ? "text-amber-200" : "text-white"}`}>{clock(remaining)}</p><p className="mt-3 text-xs leading-5 text-slate-400">{live ? `Recording ${clock(seconds)} · Microphone connected${hasVideo ? " · Camera on" : ""}` : url ? `Recorded ${clock(seconds)}. Play it back before submitting.` : "Start recording only when you are ready."}</p>{live && <div className="mt-4"><div className="flex justify-between text-[11px] text-rose-100/70"><span>Microphone</span><span>{microphoneLevel > 0.02 ? "Signal detected" : "Speak to test"}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-rose-300 transition-[width]" style={{ width: `${Math.round(microphoneLevel * 100)}%` }} /></div></div>}<div className="mt-6 flex flex-wrap gap-2"><Button onClick={live ? onStop : onRecord} className={live ? "bg-rose-500 text-white hover:bg-rose-400" : "bg-blue-500 text-white hover:bg-blue-400"}>{live ? <><CircleStop className="mr-2 h-4 w-4" />End presentation</> : <><Mic className="mr-2 h-4 w-4" />{url ? "Record again" : "Start recording"}</>}</Button>{url && <Button variant="outline" onClick={onReview} className="border-white/15 text-slate-200">Review submission <ArrowRight className="ml-2 h-4 w-4" /></Button>}</div>{url && (hasVideo ? <video className="mt-4 w-full rounded-xl" controls src={url} /> : <audio className="mt-4 w-full" controls src={url} />)}</div><div className="space-y-4">{hasVideo && <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/35"><video ref={previewRef} muted playsInline autoPlay className="aspect-video w-full object-cover" /><p className="border-t border-white/10 px-3 py-2 text-[11px] text-slate-400">Live camera preview. Visual delivery is preserved for playback only unless a validated visual-analysis service is available.</p></div>}<div className="rounded-2xl border border-blue-300/20 bg-blue-400/[0.05] p-5"><p className="font-mono-data text-[10px] uppercase tracking-[0.16em] text-blue-200/70">Judge follow-up</p><p className="mt-3 text-base leading-7 text-white">“{prompt || "Begin your presentation when you are ready."}”</p><p className="mt-3 text-xs leading-5 text-blue-100/60">The judge does not coach during evaluation. Your recording, timing, audio delivery, and supporting transcript are retained with this private attempt.</p></div>{mode === "coach" && <div className="rounded-2xl border border-violet-300/20 bg-violet-400/[0.05] p-4"><p className="text-sm font-semibold text-violet-100">Optional reflection for the next question</p><p className="mt-1 text-xs leading-5 text-violet-100/60">This note guides the next question but is not included in your score.</p><textarea value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="What have you already addressed?" className="mt-3 min-h-24 w-full rounded-xl border border-white/10 bg-black/20 p-3 text-xs leading-5 text-slate-200 outline-none focus:border-violet-300/40" /></div>}<Button disabled={!live || busy} variant="outline" onClick={onFollowUp} className="border-blue-300/25 text-blue-100 hover:bg-blue-400/10">{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}Ask next judge question</Button></div></div></section>;
}
...
function Results({ detail, loading, onNew, onDelete, deleting }: any) {
  ...
  const deliveryUnavailable = !delivery?.analysisAvailable;
