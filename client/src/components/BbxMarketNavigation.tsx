...
export function BbxMarketNavigation() {
  ...
  return (
    <nav
      ...
      className="sticky top-3 z-40 mb-7 flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-950/85 p-3 shadow-[0_20px_70px_rgba(0,0,0,.22)] backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between"
    >
      <button className="flex items-center gap-3 px-2 text-left" onClick={() => setLocation("/market")}>
      ...
      <div className="flex min-w-0 gap-1 overflow-x-auto rounded-xl border border-white/10 bg-white/[0.025] p-1">
        {marketTabs.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`inline-flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm transition ${active ? "bg-white text-slate-950 shadow-sm" : "text-foreground/60 hover:bg-white/[0.06] hover:text-foreground"}`}
