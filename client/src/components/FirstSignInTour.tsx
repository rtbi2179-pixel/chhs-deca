...
export function FirstSignInTour() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent showCloseButton={false} className="overflow-hidden border-blue-300/25 bg-slate-950 p-0 text-white shadow-[0_30px_120px_oklch(0_0_0/0.58)] sm:max-w-3xl" onEscapeKeyDown={(event) => { event.preventDefault(); startSkipExit(); }}>
        <AnimatePresence initial={false} onExitComplete={() => { if (isSkipping) finishTour(); }}>
          {!isSkipping && <motion.div key="onboarding-tour" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: ONBOARDING_SKIP_FADE_DURATION_MS / 1000, ease: "easeOut" }} className="relative grid min-h-[450px] sm:grid-cols-[0.84fr_1.16fr]">
          <div className="relative overflow-hidden border-b border-white/10 bg-[linear-gradient(155deg,oklch(0.18_0.085_253),oklch(0.07_0.02_265))] p-7 sm:border-b-0 sm:border-r sm:p-9">
            ...
            <div className="relative flex h-full min-h-[140px] flex-col justify-between">
              <div className="flex items-center justify-between"><span className="font-mono-data text-[10px] tracking-[0.19em] text-blue-100/70">BLUE BLAZER START</span><span className="rounded-full border border-blue-300/15 bg-slate-950/30 px-2 py-1 font-mono-data text-[9px] tracking-[0.12em] text-blue-200/65">0{progress.currentStep} / 0{progress.totalSteps}</span><button type="button" aria-label={ONBOARDING_TOUR_ACTIONS.skipLabel} onClick={startSkipExit} disabled={completeOnboarding.isPending || isCelebrating || isSkipping} className="text-white/45 transition-colors hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-30"><X className="h-4 w-4" /></button></div>
              <div className="flex h-24 w-24 items-center justify-center rounded-[1.5rem] border border-blue-300/30 bg-[linear-gradient(145deg,oklch(0.32_0.13_252/0.28),oklch(0.1_0.035_263/0.75))] text-blue-100 shadow-[0_0_0_12px_oklch(0.55_0.16_250/0.05),0_0_48px_oklch(0.55_0.16_250/0.25)]"><StepIcon className="h-10 w-10" /></div>
