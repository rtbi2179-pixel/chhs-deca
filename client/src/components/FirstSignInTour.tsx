...
export function FirstSignInTour() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent showCloseButton={false} className="overflow-hidden border-blue-300/20 bg-slate-950 p-0 text-white sm:max-w-2xl" onEscapeKeyDown={(event) => { event.preventDefault(); startSkipExit(); }}>
        <AnimatePresence initial={false} onExitComplete={() => { if (isSkipping) finishTour(); }}>
          {!isSkipping && <motion.div key="onboarding-tour" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: ONBOARDING_SKIP_FADE_DURATION_MS / 1000, ease: "easeOut" }} className="relative grid min-h-[430px] sm:grid-cols-[0.78fr_1.22fr]">
