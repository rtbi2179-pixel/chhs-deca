...
describe("banking end-to-end workflow", () => {
  ...
  it("completes card application, funding, purchase, payment, rewards, statement, and final-balance updates", async () => {
    ...
    await expect(awardBlueBucks(userId, 25, "correct_first_attempt", schoolCode, 777001)).resolves.toBe(true);
    ...
    await expect(api.practice.getBlueBucksBalance()).resolves.toEqual({ balance: 5 });
