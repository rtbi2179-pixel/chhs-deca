...
export function getGuidedBlazerBuddyReply(message: string): string {
  ...
  if (/(pi|performance indicator|study library|flashcard|lesson|teach.?back)/.test(question)) return "Open PI Study Library, choose your event, and start with its mapped Performance Indicators. Each module focuses on concise main ideas, flashcards, a quiz, and business scenarios.";
